...
from judge_skip import skip_pass_branch, assemble_for_judge, present_list, present_detail
...
def get_batch(batch_id: int, _user: dict = Depends(current_user)):
    with connect() as conn:
        row = conn.execute(
            "SELECT id, herb, doc, verdict, reason, created_by FROM batches WHERE id=%s",
            (batch_id,),
        ).fetchone()
    if not row:
